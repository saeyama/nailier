...
RSpec.describe Design, type: :model do
  ...
  context '画像を登録する場合' do
    it '画像が5MGまでなら有効な状態である' do
      image = build(:design, :with_5mg_jpg_image, user:)
      expect(image).to be_valid
    end

    it '画像が5MG超だと無効な状態である' do
      image = create(:design, :with_10mg_jpg_image, user:)
      expect(image).to be_invalid
    end

    it '画像の拡張子がjpeg、jpg、png以外だと無効な状態である' do
      image = create(:design, :with_pdf_image, user:)
      expect(image).to be_invalid
    end

    it '画像が8枚以下であれば有効である' do
      images = create(:design, :with_8_images, user:)
      expect(images).to be_valid
    end

    it '画像が9枚以上だと無効である' do
      images = create(:design, :with_9_images, user:)
